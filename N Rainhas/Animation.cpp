...
void Animation::Add(unsigned int id, std::vector<Rect> sequence, bool loop, bool continuous)
{
...
void Animation::Add(unsigned int id, Rect sequence)
{
...
bool Animation::Run(unsigned int id, float frameTime)
{
    ...
    if (currentId != id)
    {
        ...
        if (sequence != rectMap.end())
        {
            ...
            // Reinicializa o índice dependendo da continuidade da animação
